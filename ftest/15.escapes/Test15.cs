// Machine generated by peg-sharp 0.3.211.0 from Test15.peg.
using System;
...
internal sealed partial class Test15
{
	...
	private State DoParseExprRule(State _state, List<Result> _outResults)
	{
		...
		_state = DoChoice(_state, results,
			(s, r) => DoSequence(s, r,
				(s2, r2) => DoParseLiteral(s2, r2, "a"),
				(s2, r2) => DoParseRange(s2, r2, false, string.Empty, "\x0001\xFFFF", null, ".")),
			(s, r) => DoParseLiteral(s, r, "b\x41"),
			(s, r) => DoParseRange(s, r, false, "c\x42", string.Empty, null, "[c\x42]"),
			(s, r) => DoParseRange(s, r, false, "d", string.Empty, new UnicodeCategory[]{UnicodeCategory.DashPunctuation}, "[d\\cPd]"),
			(s, r) => DoParseLiteral(s, r, "z"));
	...
	private State DoParseRange(State state, List<Result> results, bool inverted, string chars, string ranges, UnicodeCategory[] categories, string label)
	{
		...
		if (matched)
		{
			...
			results.Add(new Result(this, state.Index, text, default(double)));
	...
	{
		public Result(Test15 parser, int index, string text, double value)
		{
			m_parser = parser;
			m_index = index;
			m_text = text;
