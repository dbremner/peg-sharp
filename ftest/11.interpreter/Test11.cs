// Machine generated by peg-sharp 0.1.830.0 from Test11.peg.
using System;
...
internal sealed partial class Test11
{
	...
	private State DoParseProgramRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	private State DoParseAssignmentRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	private State DoParseIfRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	private State DoParseStatementRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	private State DoParseStatementsRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	private State DoParseWhileRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	// Compare := Sum (('<=' / '>=' / '==' / '!=' / '<' / '>') S Sum)?
	private State DoParseCompareRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	private State DoParseExpressionRule(State state, List<Result> outResults)
	{
		...
		state = DoParse(state, results, "Compare");
	...
	private State DoParseValue2Rule(State state, List<Result> outResults)
	{
		...
		state = DoParse(state, results, "Identifier");
	...
	// Value := '(' Expression ')' S
	private State DoParseValue3Rule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	private State DoParseKeywordsRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	private State DoParse(State state, List<Result> results, string nonterminal)
	{
		...
		if (!m_cache.TryGetValue(key, out cache))
		{
			...
			Expression value = state.Parsed && results.Count > oldCount ? results[results.Count - 1].Value : default(Expression);
	...
	{
		public CacheValue(State state, Expression value)
		{
			State = state;
			Value = value;
		}
