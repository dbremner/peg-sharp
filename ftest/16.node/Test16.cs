// Machine generated by peg-sharp 0.2.206.0 from Test16.peg.
using System;
...
internal sealed class Node
{
	...
	public bool NonTerminal {get; set;}
	...
	public override string ToString()
	{
		if (NonTerminal)
			...
			return string.Format("'{0}' literal", Text);
	}
}
...
internal sealed partial class Test16
{
	...
	private State DoParseExprRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
		{
			...
			Node value = new Node{Text = text, Line = DoGetLine(_start.Index), Col = DoGetCol(_start.Index), NonTerminal = true, Name = "Expr", Children = (from r in results where r.Value != null select r.Value).ToArray()};
	...
	private State DoParseSumRule(State _state, List<Result> _outResults)
	{
		...
		{
			string text = m_input.Substring(_start.Index, _state.Index - _start.Index);
			Node value = new Node{Text = text, Line = DoGetLine(_start.Index), Col = DoGetCol(_start.Index), NonTerminal = true, Name = "Sum", Children = (from r in results where r.Value != null select r.Value).ToArray()};
			_outResults.Add(new Result(this, _start.Index, text, value));
		}
	...
	private State DoParseProductRule(State _state, List<Result> _outResults)
	{
		...
		{
			string text = m_input.Substring(_start.Index, _state.Index - _start.Index);
			Node value = new Node{Text = text, Line = DoGetLine(_start.Index), Col = DoGetCol(_start.Index), NonTerminal = true, Name = "Product", Children = (from r in results where r.Value != null select r.Value).ToArray()};
			_outResults.Add(new Result(this, _start.Index, text, value));
		}
	...
	private State DoParseValue1Rule(State _state, List<Result> _outResults)
	{
		...
		{
			string text = m_input.Substring(_start.Index, _state.Index - _start.Index);
			Node value = new Node{Text = text, Line = DoGetLine(_start.Index), Col = DoGetCol(_start.Index), NonTerminal = true, Name = "Value", Children = (from r in results where r.Value != null select r.Value).ToArray()};
			_outResults.Add(new Result(this, _start.Index, text, value));
		}
		else
		{
	...
	private State DoParseSRule(State _state, List<Result> _outResults)
	{
		...
		{
			string text = m_input.Substring(_start.Index, _state.Index - _start.Index);
			Node value = new Node{Text = text, Line = DoGetLine(_start.Index), Col = DoGetCol(_start.Index), NonTerminal = true, Name = "S", Children = (from r in results where r.Value != null select r.Value).ToArray()};
			text = null;
			if (text != null)
				_outResults.Add(new Result(this, _start.Index, text, value));
		}
	...
	private State DoParseSpaceRule(State _state, List<Result> _outResults)
	{
		...
		{
			string text = m_input.Substring(_start.Index, _state.Index - _start.Index);
			Node value = new Node{Text = text, Line = DoGetLine(_start.Index), Col = DoGetCol(_start.Index), NonTerminal = true, Name = "Space", Children = (from r in results where r.Value != null select r.Value).ToArray()};
			_outResults.Add(new Result(this, _start.Index, text, value));
		}
		else
		{
	...
	private State DoParseLiteral(State state, List<Result> results, string literal)
	{
		...
		results.Add(new Result(this, j, literal, new Node{Text = literal, Line = DoGetLine(j), Col = DoGetCol(j), NonTerminal = false}));
	...
	private State DoParseRange(State state, List<Result> results, bool inverted, string chars, string ranges, UnicodeCategory[] categories, string label)
	{
		...
		if (matched)
		{
			string text = m_input.Substring(state.Index, 1);
			results.Add(new Result(this, state.Index, text, new Node{Text = text, Line = DoGetLine(state.Index), Col = DoGetCol(state.Index), NonTerminal = false}));
	...
	{
		public Result(Test16 parser, int index, string text, Node value)
		{
			m_parser = parser;
			m_index = index;
			m_text = text;
