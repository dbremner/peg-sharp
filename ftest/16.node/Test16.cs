// Machine generated by peg-sharp 0.3.346.0 from Test16.peg.
using System;
...
internal sealed partial class Test16
{
	...
	private State DoParseExprRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
		{
			XmlNode value = DoCreateElementNode("Expr", _start.Index, _state.Index - _start.Index, DoGetLine(_start.Index), DoGetCol(_start.Index), (from r in results where r.Value != null select r.Value).ToArray());
			_outResults.Add(new Result(this, _start.Index, _state.Index - _start.Index, m_input, value));
		}
		
		if (_state.Parsed)
			DoDebugWrite("Expr parsed: {0}", DoTruncateString(m_input.Substring(_start.Index, _state.Index - _start.Index)));
		else
			DoDebugWrite("Expr failed: {0} at line {1} col {2}", _state.Errors, DoGetLine(_state.Errors.Index), DoGetCol(_state.Errors.Index));
		--m_debugLevel;
	...
	private State DoParseSumRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
		{
			XmlNode value = DoCreateElementNode("Sum", _start.Index, _state.Index - _start.Index, DoGetLine(_start.Index), DoGetCol(_start.Index), (from r in results where r.Value != null select r.Value).ToArray());
			_outResults.Add(new Result(this, _start.Index, _state.Index - _start.Index, m_input, value));
		}
		
		if (_state.Parsed)
			DoDebugWrite("Sum parsed: {0}", DoTruncateString(m_input.Substring(_start.Index, _state.Index - _start.Index)));
		else
			DoDebugWrite("Sum failed: {0} at line {1} col {2}", _state.Errors, DoGetLine(_state.Errors.Index), DoGetCol(_state.Errors.Index));
		--m_debugLevel;
	...
	private State DoParseProductRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
		{
			XmlNode value = DoCreateElementNode("Product", _start.Index, _state.Index - _start.Index, DoGetLine(_start.Index), DoGetCol(_start.Index), (from r in results where r.Value != null select r.Value).ToArray());
			_outResults.Add(new Result(this, _start.Index, _state.Index - _start.Index, m_input, value));
		}
		
		if (_state.Parsed)
			DoDebugWrite("Product parsed: {0}", DoTruncateString(m_input.Substring(_start.Index, _state.Index - _start.Index)));
		else
			DoDebugWrite("Product failed: {0} at line {1} col {2}", _state.Errors, DoGetLine(_state.Errors.Index), DoGetCol(_state.Errors.Index));
		--m_debugLevel;
	...
	private State DoParseValue1Rule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
		{
			XmlNode value = DoCreateElementNode("Value", _start.Index, _state.Index - _start.Index, DoGetLine(_start.Index), DoGetCol(_start.Index), (from r in results where r.Value != null select r.Value).ToArray());
		...
		if (_state.Parsed)
			DoDebugWrite("Value parsed: {0}", DoTruncateString(m_input.Substring(_start.Index, _state.Index - _start.Index)));
		else
			DoDebugWrite("Value failed: {0} at line {1} col {2}", _state.Errors, DoGetLine(_state.Errors.Index), DoGetCol(_state.Errors.Index));
		--m_debugLevel;
	...
	private State DoParseSRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
		{
			XmlNode value = DoCreateElementNode("S", _start.Index, _state.Index - _start.Index, DoGetLine(_start.Index), DoGetCol(_start.Index), (from r in results where r.Value != null select r.Value).ToArray());
			string text = m_input.Substring(_start.Index, _state.Index - _start.Index);
			text = null;
			if (text != null)
				_outResults.Add(new Result(this, _start.Index, _state.Index - _start.Index, m_input, value));
		...
		if (_state.Parsed)
			DoDebugWrite("S parsed: {0}", DoTruncateString(m_input.Substring(_start.Index, _state.Index - _start.Index)));
		else
			DoDebugWrite("S failed: {0} at line {1} col {2}", _state.Errors, DoGetLine(_state.Errors.Index), DoGetCol(_state.Errors.Index));
		--m_debugLevel;
	...
	private State DoParseSpaceRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
		{
			XmlNode value = DoCreateElementNode("Space", _start.Index, _state.Index - _start.Index, DoGetLine(_start.Index), DoGetCol(_start.Index), (from r in results where r.Value != null select r.Value).ToArray());
			_outResults.Add(new Result(this, _start.Index, _state.Index - _start.Index, m_input, value));
		}
		else
		{
		...
		if (_state.Parsed)
			DoDebugWrite("Space parsed: {0}", DoTruncateString(m_input.Substring(_start.Index, _state.Index - _start.Index)));
		else
			DoDebugWrite("Space failed: {0} at line {1} col {2}", _state.Errors, DoGetLine(_state.Errors.Index), DoGetCol(_state.Errors.Index));
		--m_debugLevel;
	...
	private string[] m_debug = new string[]{"*"};
