// Machine generated by peg-sharp 0.4.713.0 from 01.basic-literals/Test1.peg.
using System;
...
internal sealed partial class Test1
{
	public Test1()
	{
	...
	private State DoParseExprRule(State _state, List<Result> _outResults)
	{
		...
		_state = DoChoice(_state, results,
			...
			delegate (State s, List<Result> r) {return DoParseLiteral(s, r, "'D'");});
	...
	private State DoParseLiteral(State state, List<Result> results, string literal)
	{
		...
		{
			results.Add(new Result(this, state.Index, literal.Length, m_input, default(int)));
	...
	{
		public Result(Test1 parser, int index, int length, string input, int value)
		{
			m_parser = parser;
			m_index = index;
			m_length = length;
