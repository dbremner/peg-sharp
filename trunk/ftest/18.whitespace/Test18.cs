// Machine generated by peg-sharp 0.4.791.0 from ftest/18.whitespace/Test18.peg.
using System;
...
internal sealed partial class Test18
{
	...
	private State DoParseFunctionRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "Function parsed");
		else
			DoDebug(_state.Index, _start.Index, "Function " + _state.Errors);
	...
	private State DoParseKeywordsRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "Keywords parsed");
		else
			DoDebug(_state.Index, _start.Index, "Keywords " + _state.Errors);
	...
	private State DoParseIfStatementRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "IfStatement parsed");
		else
			DoDebug(_state.Index, _start.Index, "IfStatement " + _state.Errors);
	...
	private State DoParseIdentifierRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "Identifier parsed");
		else
			DoDebug(_state.Index, _start.Index, "Identifier " + _state.Errors);
	...
	private State DoParsePassStatementRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "PassStatement parsed");
		else
			DoDebug(_state.Index, _start.Index, "PassStatement " + _state.Errors);
	...
	private State DoParseProgramRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "Program parsed");
		else
			DoDebug(_state.Index, _start.Index, "Program " + _state.Errors);
	...
	private State DoParseSRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "S parsed");
		else
			DoDebug(_state.Index, _start.Index, "S " + _state.Errors);
	...
	private State DoParseSpaceRule(State _state, List<Result> _outResults)
	{
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "Space parsed");
		else
			DoDebug(_state.Index, _start.Index, "Space " + _state.Errors);
	...
	private State DoParseStatementRule(State _state, List<Result> _outResults)
	{
		...
		if (fail != null)
		{
			DoDebug(_state.Index, _start.Index, "Statement failed prolog");
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "Statement parsed");
		else
			DoDebug(_state.Index, _start.Index, "Statement " + _state.Errors);
	...
	private State DoParseStatementsRule(State _state, List<Result> _outResults)
	{
		...
		if (fail != null)
		{
			DoDebug(_state.Index, _start.Index, "Statements failed prolog");
			return new State(_start.Index, false, ErrorSet.Combine(_start.Errors, new ErrorSet(_start.Index, fail)));
		}
		...
		if (_state.Parsed)
			DoDebug(_start.Index, _state.Index, "Statements parsed");
		else
			DoDebug(_state.Index, _start.Index, "Statements " + _state.Errors);
