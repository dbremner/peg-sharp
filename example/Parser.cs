// Machine generated by peg-sharp 0.1.22.0 from example/Parser.peg.
using System;
...
internal sealed partial class Parser
{
	...
	private State DoParseStartRule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
		...
		state = DoSequence(state, results,
			(s, r) => DoParse(s, r, "S"),
			(s, r) => DoChoice(s, r,
				(s2, r2) => DoParse(s2, r2, "Assignment"),
				(s2, r2) => DoParse(s2, r2, "Expression")));
	...
	private State DoParseExpressionRule(State state, List<Result> outResults)
	{
		...
		state = DoParse(state, results, "Sum");
	...
	private State DoParseIdentifierRule(State state, List<Result> outResults)
	{
		...
		state = DoSequence(state, results,
			(s, r) => DoParseRange(s, r, false, "$", "azAZ", null, "[$a-zA-Z]"),
	...
	private State DoParseValue1Rule(State state, List<Result> outResults)
	{
		...
		state = DoSequence(state, results,
			...
			(s, r) => DoRepetition(s, r, 0, 1,
				(s2, r2) => DoSequence(s2, r2,
					(s3, r3) => DoChoice(s3, r3,
						(s4, r4) => DoParseLiteral(s4, r4, "e"),
						(s4, r4) => DoParseLiteral(s4, r4, "E")),
					(s3, r3) => DoRepetition(s3, r3, 1, 2147483647,
						(s4, r4) => DoParseRange(s4, r4, false, string.Empty, "09", null, "[0-9]")))),
			(s, r) => DoParse(s, r, "S"));
	...
	// Value := Identifier
	private State DoParseValue4Rule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	// Value := '(' Expression ')' S
	private State DoParseValue5Rule(State state, List<Result> outResults)
	{
		State start = state;
		var results = new List<Result>();
	...
	{
		public Result(Parser parser, int index, string text, Expression value)
		{
			m_parser = parser;
			m_index = index;
			m_text = text;
